...
export class RegisterComponent implements OnInit {
  ...
  zone_given: boolean = false
  region_given: boolean = false
  location_given: boolean = false
      ...
      this.http.post(this.service_name_user, this.request2, httpOptions)
        ...
        .subscribe(data => {
          ...
          if (this.response2['status'] && this.response2['status'] == "Success") {
            ...
            this.name = this.response2['name']
            this.title = this.response2['title']
            this.role_id = this.response2['role_id']
            this.username = this.emp_id
            this.zone = this.response2['zone']
            if (this.zone) {
              this.zone_given = true
            }
            this.region = this.response2['region']
            if (this.region) {
              this.region_given = true
            }
            this.location = this.response2['location']
            if (this.location) {
              this.location_given = true
            }
          }
          else if (this.response2['status'] && this.response2['status'] == "Exists") {
